...
const MovieAlternativeTitle = () => {
    ...
    const [availCountries, setavailCountries] = useState("")
    const url = `https://api.themoviedb.org/3/movie/${param.id}/alternative_titles?api_key=184d4ae5b2d8ff18eca3972237dc1b9f&language=en-US`

    const loadApi = async () => {
        const fetchapi = await fetch(url)
        const response = await fetchapi.json()
        setresult(response.titles)
        return response.titles
    }

    useEffect(() => {
        loadApi().then((result)=>{
            result && setavailCountries(result.map((e)=>e.iso_3166_1).toString())
        })
    ...
    return (
        <div>
            ...
            {result && <div className='container'>
                {countries.filter((el)=>availCountries.includes(el.iso_3166_1)).map((value) => {
                    return (
                         <div><div className="card mb-4">
                            <div className="card-header">
                            <img className='me-2' src={`https://flagcdn.com/24x18/${value.iso_3166_1.toLowerCase()}.png`} alt=""/><b>{value.english_name}</b>
                            </div>
                            <div className="card-body">

                                <div className='row'>
                                    <div className='col-md-6'>
                                        <b> <p className="card-title mb-0">Title</p></b>
                                    </div>

                                    <div className='col-md-6'>
                                        <b><p className="card-title mb-0">Type</p></b>
                                    </div>
                                </div>
                                <hr className='my-1' />
                                {result.filter((ele) => ele.iso_3166_1 === value.iso_3166_1).map((e) => {
                            return (
                                <div className='row'>
                                    <div className='col-md-6'>
                                        <p className="card-text mb-0">{e.title}</p>
                                    </div>

                                    <div className='col-md-6'>
                                        <p className="card-text mb-0">{e.type}</p>
                                    </div>
                                </div>
                            )})    
                            }
...
export default MovieAlternativeTitle